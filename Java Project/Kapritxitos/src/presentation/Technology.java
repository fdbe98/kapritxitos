...
public class Technology extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)	throws ServletException, IOException {
		...
		String brand = request.getParameter("selMar"); 
		...
		if(price.equals(""))
			price = "100000";
		//Check if brand is "Anyone"
		if(brand.equals("Cualquiera"))
			brand = "";
		...
		if(!checkcoord) {
			...
			String[][] orderedList = Distance.getApiDistance("tec",latitude, longitude, transport);
			for(int i=0;i<orderedList.length;i++)
				System.out.println("Shop: "+orderedList[i][0]+": Value: "+orderedList[i][1]+", Time: "+orderedList[i][2]+" Distance: "+orderedList[i][3]);
			
			try {
				
				inventory = Methods.readJSONfileTec(type);
				out.println(showTable.showtableXML_JSONTec(num, inventory, orderedList, type, brand, desc,Float.parseFloat(price)));
			...
			try {
				...
				inventory = Methods.shopsOrderedByPrice(Methods.readJSONfileTec(type));
				System.out.println("FICHERO LEIDO");
				out.println(showTable.showtablePriceTec(num, inventory, type, brand, desc, Float.parseFloat(price)));
			} catch (JAXBException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
